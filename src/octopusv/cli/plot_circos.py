...
def _load_circos_backend():
    """Load the Circos plotting backend only when plot-circos is actually run.

    This keeps `octopusv -h` and `octopusv plot-circos -h` lightweight and avoids
    triggering matplotlib font-cache messages or pycirclize import errors unless
    ...
    try:
        ...
        if exc.name == "pycirclize":
            typer.echo(
                "Error: pycirclize is required for `octopusv plot-circos`.\n"
    ...
    return CircosPlotter, load_chrom_sizes_from_fai
...
def plot_circos(
    ...
    fai: Path = typer.Option(
        ...
        "--fai",
        help=(
            "Reference FASTA .fai index for chromosome sizes. "
    ...
    require_pass: bool = typer.Option(
        ...
        "--require-pass",
        help=(
            "Keep only FILTER=PASS records. Off by default, because OctopuSV "
    ...
    intra_max_span: int = typer.Option(
        ...
        "--intra-max-span",
        help=(
            "Maximum span (bp) for intra links. Larger events go to an "
    ...
    bin_size: int = typer.Option(
        5_000_000,
        "--bin",
        help="Breakpoint density histogram bin size (bp).",
    ),

    # ---- type switches ----
    tra_only: bool = typer.Option(
        False,
        "--tra-only",
        help="Plot only TRA links.",
    ),
    intra_only: bool = typer.Option(
        False,
        "--intra-only",
        help="Plot only DEL/DUP/INV links.",
    ),
    drop_del: bool = typer.Option(
        False,
        "--drop-del",
        help="Drop DEL links.",
    ),
    drop_dup: bool = typer.Option(
        False,
        "--drop-dup",
        help="Drop DUP links.",
    ),
    drop_inv: bool = typer.Option(
        False,
        "--drop-inv",
        help="Drop INV links.",
    ),
    ...
    # ---- rendering ----
    tra_alpha: float = typer.Option(
        0.30,
        "--tra-alpha",
        help="TRA link transparency.",
    ),
    intra_alpha: float = typer.Option(
        0.65,
        "--intra-alpha",
        help="Intra link transparency.",
    ),
    link_lw: float = typer.Option(
        0.6,
        "--link-lw",
        help="Intra link line width.",
    ),
    tra_lw: float = typer.Option(
        1.1,
        "--tra-lw",
        help="TRA link line width (thicker).",
    ),
    intra_height: float = typer.Option(
        0.30,
        "--intra-height",
        help="Intra arc height ratio (lower = flatter, near rim).",
    ),
    tra_height: float = typer.Option(
        0.55,
        "--tra-height",
        help="TRA arc height ratio (higher = bulges through center).",
    ),
    ...
    """
    # Import plotting backend only when the command actually runs, not when
    # the top-level CLI help is rendered.
    CircosPlotter, load_chrom_sizes_from_fai = _load_circos_backend()
